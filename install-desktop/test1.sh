...
configure_sddm() {
    ...
    if [ -n "$current_manager" ]; then
        sudo systemctl disable "$current_manager" || \
            echo "Cannot disable current display manager."
    fi
    ...
    if [ -f "/usr/lib/sddm/sddm.conf.d/default.conf" ]; then
        ...
        sudo cp \
            ...
            /usr/lib/sddm/sddm.conf.d/archnoobtheme.conf
        ...
        if [ -f "/usr/lib/sddm/sddm.conf.d/archnoobtheme.conf" ]; then
            ...
            sudo sed -i \
                's/^Current=.*/Current=archnoobtheme/' \
                /etc/sddm.conf.d/archnoobtheme.conf
        fi
    fi
